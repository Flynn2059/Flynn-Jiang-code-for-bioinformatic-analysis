...
# ==== 加载数据 ====
seurat_obj=ReadMtx(
  mtx="GSM8899426_X10_19_matrix.mtx.gz",
  cells = "GSM8899426_X10_19_barcodes.tsv.gz",
  features = "GSM8899426_X10_19_features.tsv.gz",
  feature.column = 2
)
seurat_obj=CreateSeuratObject(seurat_obj,project = "GSM8899426",min.cells = 10,min.features = 50) 
...
VlnPlot(seurat_obj, features = c("nFeature_RNA", "nCount_RNA", "percent_mt", "percent_RBC"), 
        pt.size = 0,ncol = 2)
# 进行质控，标准如下
seurat_obj=subset(seurat_obj,subset = nFeature_RNA>500 & percent_mt<25  & percent_RBC<1)
print(seurat_obj)
...
seurat_obj=FindNeighbors(seurat_obj,dims = 1:9) %>% FindClusters(resolution=0.1)
# 找合适的pK数
possible_pK=paramSweep(seurat_obj,PCs=1:9,sct = F)
## PCs：主成分，上面的值
...
best_pK=0.26
# 标记双细胞并且查看结果
doublet_rate=(ncol(seurat_obj)/1000)*0.004
nExp=round(doublet_rate*ncol(seurat_obj)) # 10X平台的估计双细胞数目
seurat_obj=DoubletFinder::doubletFinder(seurat_obj,PCs = 1:9,pN = 0.25,pK = best_pK,
                                        nExp = nExp,sct = F)
colnames(seurat_obj@meta.data)
table(seurat_obj@meta.data[["DF.classifications_0.25_0.26_355"]])
seurat_obj=subset(seurat_obj,DF.classifications_0.25_0.26_355=="Singlet")
print(seurat_obj)
qsave(seurat_obj,file = "GSM8899426_post_QC.qs")
